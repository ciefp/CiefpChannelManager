#!/bin/bash
##setup command=wget -q "--no-check-certificate" https://raw.githubusercontent.com/ciefp/CiefpChannelManager/main/installer.sh -O - | /bin/sh
...
TMPPATH=/tmp/CiefpChannelManager

if [ ! -d /usr/lib64 ]; then
	PLUGINPATH=/usr/lib/enigma2/python/Plugins/Extensions/CiefpChannelManager
else
	PLUGINPATH=/usr/lib64/enigma2/python/Plugins/Extensions/CiefpChannelManager
fi
...
if [ -f /var/lib/dpkg/status ]; then
   ...
   wget https://github.com/ciefp/CiefpChannelManager/archive/refs/heads/main.tar.gz
   tar -xzf main.tar.gz
   cp -r 'CiefpChannelManager-main/usr' '/'
set +e
cd
sleep 2
